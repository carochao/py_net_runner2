...
for (const file of files) {
  ...
  const listFilteringBlock = `  "list-filtering": {
    ...
    technical: "### Comprehension Syntax:\\n- Comprehensions allow single-line loops: \`[elem for elem in list if condition]\`.\\n- **Dynamic Loop Variable**: You can choose any valid loop variable name for \`elem\` (such as \`x\`, \`val\`, or \`item\`) as long as you use it consistently throughout the expression.\\n- It constructs and returns a fully new list on the fly.",
    ...
    solutionRegex: [/${config.filterDest}\\s*=\\s*\\[\\s*([a-zA-Z_]\\w*)\\s+for\\s+\\1\\s+in\\s+${config.filterListVar}\\s+if\\s+\\1\\s*>\\s*50\\s*\\]/, /print\\s*\\(\\s*${config.filterDest}\\s*\\)/]
  }`;
  ...
  const listComprehensionRangeBlock = `  "list-comprehension-range": {
    ...
    technical: "### Modular Filtration:\\n- Check if indices are divisible: \`elem % 2 == 0\` catches all even integers.\\n- **Dynamic Loop Variable**: You can choose any valid loop variable name for \`elem\` (such as \`x\`, \`val\`, or \`num\`) as long as you use it consistently throughout the expression.\\n- Efficiently applies complex criteria without long multiline nested loops.",
    ...
    solutionRegex: [/${config.rangeDest}\\s*=\\s*\\[\\s*([a-zA-Z_]\\w*)\\s+for\\s+\\1\\s+in\\s+${config.rangeListVar}\\s+if\\s+\\1\\s*%\\s*2\\s*==\\s*0\\s*\\]/, /print\\s*\\(\\s*${config.rangeDest}\\s*\\)/]
  }`;
