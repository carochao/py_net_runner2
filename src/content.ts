  ...
  },
{
    ...
    description: "Understand the core concept of Object-Oriented Programming (OOP) with intuitive analogies.",
    headerPrefix: "OOP_CORE",
    missionPrefix: "BLUEPRINT_GEN",
    intro: "# What is Object-Oriented Programming (OOP)? 🧱\n\nThink about your favorite video game—like Minecraft, Pokémon, or Mario. There isn't just *one* block, *one* Pokémon, or *one* coin. There are dozens, hundreds, or thousands of them!\n\nBefore OOP existed, programmers had to create separate, loose variables for every single character in the game:\n```python\nplayer1_name = \"Alex\"\nplayer1_health = 100\nplayer2_name = \"Sam\"\nplayer2_health = 80\n```\nIf you had 100 players, you would need 200 loose variables! Your code quickly turned into an unmanageable spiderweb of tangled data.\n\n**Object-Oriented Programming (OOP)** is a design pattern that solves this problem by packaging related data (variables) and actions (functions) together into neat, reusable bundles called **Objects**.\n\n### The Cookie Cutter Analogy 🍪\n- **The Class is the Cookie Cutter (The Blueprint)**: It's the metal shape. It is not an edible cookie itself—it just defines what a cookie will look like!\n- **The Object is the Actual Cookie (The Instance)**: When you stamp the cutter into dough, you create a real, edible cookie. You can stamp out 100 cookies from that single cutter. Each cookie can have its own frosting, sprinkles, or toppings, but they all share the exact same shape.",
    technical: "### How to Build a Class in Python ✏️\n\nCreating a class in Python is simple and follows a clean 3-part formula:\n\n1. **The `class` Keyword**:\n   Just like defining a function starts with `def`, creating a class starts with the keyword `class`, followed by the name you want to give your blueprint, and a colon `:`.\n   ```python\n   class Operative:\n   ```\n   *💡 Naming Tip*: By standard Python convention, class names use **PascalCase** (every word starts with a capital letter, like `Operative`, `GamePlayer`, or `SpaceStation`), whereas normal variables use lowercase `snake_case`.\n\n2. **Indentation (4 Spaces)**:\n   Python requires indentation (4 spaces or 1 Tab) to know what code belongs inside the class blueprint.\n\n3. **The `pass` Keyword (Safe Placeholder)**:\n   In Python, an indented code block cannot be completely empty! If you write `class Operative:` and leave the next line blank, Python crashes with an `IndentationError`.\n   Writing **`pass`** literally tells Python: *\"Nothing to do here right now, I'm just setting up an empty placeholder! Keep going!\"*\n\n### How to Stamp Out a Real Object (Instantiation):\nOnce your class blueprint is defined, creating a real object from it is as easy as calling a function:\n```python\nagent1 = Operative()  # Stamped out our first real operative!\nagent2 = Operative()  # Stamped out a second operative!\n```",
    example: "# Step 1: Define the blueprint (Class):\nclass Drone:\n    pass  # Placeholder: blueprint is empty for now\n\n# Step 2: Stamp out real, tangible objects (Instances):\nbot_alpha = Drone()\nbot_beta = Drone()\n\nprint(type(bot_alpha))  # Output: <class '__main__.Drone'>\nprint(\"Drone fleet successfully initialized!\")",
    task: "### YOUR MISSION 🎯\n\nCreate your very first class blueprint!\n\n1. On line 1, use the `class` keyword to define a class named `Operative` followed by a colon `:`.\n2. On line 2, indent with **4 spaces** and write `pass` so Python knows the blueprint is ready and valid.",
    baseCode: "# TODO: Define class Operative with pass inside\n",
    hints: [
      "Type 'class Operative:' on line 1.",
      "Press Enter, indent with 4 spaces (or press Tab), and type 'pass'."
    ],
  ...
  },
{
    ...
    description: "Learn how objects receive their starting data upon birth using __init__ and self.",
    headerPrefix: "CONSTRUCTOR",
    missionPrefix: "INIT_IDENTITY",
    intro: "# The Object's Birth Certificate: __init__ 🐣\n\nWhen a new player joins a game, they don't enter the world completely blank! They immediately need a username, starting health points, and gear.\n\nIn Python, **`__init__`** is a special built-in method known as the **Constructor**. Its name comes from the word *\"initialize\"* (which means to set up or prepare starting values).\n\n### The Superpower of `__init__`:\nWhenever you stamp out a new object from a class, **Python automatically runs `__init__` behind the scenes!** You never have to call `__init__()` manually—it fires the exact millisecond the object is born.\n\n*(Those double underscores before and after are nicknamed **\"dunder\"** by programmers—short for **D**ouble **UNDER**score!)*",
    technical: "### Demystifying `self`: The Secret to OOP 💡\n\nAlmost every beginner feels confused when they first see `self`. Here is the golden mental model that makes it click immediately:\n👉 **Translate `self` as \"THIS SPECIFIC OBJECT\" or \"MY\"!**\n\nImagine you stamp out two different operatives:\n```python\nagent_a = Operative(\"Cipher\")\nagent_b = Operative(\"Viper\")\n```\nWhen Python is creating `agent_a`, `self` represents `agent_a`. When it creates `agent_b`, `self` represents `agent_b`.\n\n### Breaking Down: `self.designation = designation`\nBeginners often ask: *\"Why are we writing the word designation twice?\"*\n- **`designation`** (on the right side of `=`): The temporary incoming value passed into the constructor (like `\"Cipher\"`).\n- **`self.designation`** (on the left side of `=`): The permanent variable stored inside **THIS** object's personal memory backpack (called an **attribute** or **instance variable**).\n- **In plain English**: *\"Hey Python! Take the incoming designation that was passed in, and save it as MY permanent personal designation!\"*\n\n```python\nclass Operative:\n    def __init__(self, designation):\n        # 4 spaces inside def (8 spaces total from left margin)\n        self.designation = designation\n```",
    example: "class Pet:\n    # Runs automatically whenever Pet(...) is created\n    def __init__(self, pet_name, energy):\n        self.name = pet_name      # 'My name is...'\n        self.energy = energy      # 'My energy level is...'\n\n# Creating two different pets from the same blueprint:\ndog = Pet(\"Barnaby\", 95)\ncat = Pet(\"Luna\", 70)\n\nprint(dog.name)    # Outputs: Barnaby\nprint(cat.energy)  # Outputs: 70",
    task: "### YOUR MISSION 🎯\n\nGive your `Operative` class a constructor so every operative can store their own unique code designation!\n\n1. Indented 4 spaces inside `Operative`, define the constructor: `def __init__(self, designation):`\n2. Indented 8 spaces on the next line, assign the incoming argument to the object's attribute: `self.designation = designation`",
    baseCode: "class Operative:\n    # TODO: Add __init__ constructor\n",
    hints: [
      "Write 'def __init__(self, designation):' indented 4 spaces inside the class.",
      "Inside the constructor (indented 8 spaces), write: self.designation = designation"
    ],
    solution: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation",
  ...
  },
{
    ...
    description: "Teach your objects how to perform actions and behaviors using methods.",
    headerPrefix: "METHOD_ACT",
    missionPrefix: "EXEC_ACTION",
    intro: "# Teaching Objects How to Act: Methods ⚡\n\nSo far, our operatives only store passive information (like a name or designation). But in the real world and in video games, objects don't just hold data—**they do things**!\n\n- A Dog can **bark** and **fetch** 🐕\n- A Sports Car can **accelerate** and **drift** 🏎️\n- A Bank Account can **deposit** and **withdraw** 💳\n- An Operative can **ping the satellite** 🛰️\n\nIn Python, a function that lives inside a class is called a **Method**. Methods define the actions, abilities, and superpowers that your objects can perform!",
    technical: "### Functions vs. Methods: What's the Difference? 🔍\n\n- A standard **Function** floats freely on its own: `print()`, `len()`, `calculate()`. You call it by itself.\n- A **Method** belongs directly to an object: `dog.bark()`, `car.drive()`, `agent.ping()`. You call it using the **dot operator** (`object.method()`)!\n\n### Why does EVERY method need `(self)` as its first parameter?\nWhen you write `agent.ping()`, Python secretly translates that under the hood into:\n```python\nOperative.ping(agent)  # Python passes the object itself as the 1st argument!\n```\nBecause Python automatically injects the object into the first slot, **every method inside a class must accept `self` as its first parameter**, even if it takes zero other inputs!\n\n### ⚠️ The Classic Beginner Gotcha:\nIf you define `def ping():` without `self`, Python will crash when you run `agent.ping()` with:\n`TypeError: ping() takes 0 positional arguments but 1 was given`\nThat mysterious \"1 argument\" Python is complaining about is `self`! Always include `self` in the method header.",
    example: "class Warrior:\n    def __init__(self, name):\n        self.name = name\n\n    # A method (action) - notice 'self' in the parentheses:\n    def battle_cry(self):\n        print(f\"{self.name} charges into battle! For glory!\")\n\nhero = Warrior(\"Valkyrie\")\nhero.battle_cry()  # Outputs: Valkyrie charges into battle! For glory!",
    task: "### YOUR MISSION 🎯\n\nTeach your `Operative` how to transmit an encrypted communications ping!\n\n1. Keep the `__init__` constructor that stores `self.designation`.\n2. Indented 4 spaces inside `Operative`, define a new method named `ping`: `def ping(self):`\n3. Inside `ping` (indented 8 spaces), print the message: `\"Signal sent\"`",
    baseCode: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n    # TODO: Add ping method here\n",
    hints: [
      "Define the method with 'def ping(self):' (indented 4 spaces inside the class).",
      "Inside the ping method (indented 8 spaces), write: print(\"Signal sent\")"
    ],
    solution: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n    def ping(self):\n        print(\"Signal sent\")",
  ...
  },
{
    ...
    description: "Understand how each object maintains its own independent data and changes over time.",
    headerPrefix: "OBJ_STATE",
    missionPrefix: "STATE_TRACK",
    intro: "# Each Object Lives in Its Own World: State 🎒\n\nThink about playing an online multiplayer game with a friend. If your character takes damage and drops to 20 HP, does your friend's character lose health too? Of course not!\n\nEven though all player characters are created from the exact same `Character` class blueprint, each individual object retains its own **independent state** (its own personal snapshot of current values stored inside `self`).\n\nIn this mission, you will learn how to give objects **default starting values** and how to write methods that update an object's internal state over time!",
    technical: "### Setting and Updating Object State 🔄\n\n1. **Default Values in `__init__`**:\n   An object's variables don't all have to come from outside arguments! You can give an object default starting values directly inside `__init__`:\n   ```python\n   def __init__(self, designation):\n       self.designation = designation\n       self.status = \"Offline\"  # Every operative starts Offline automatically!\n   ```\n\n2. **Modifying State Inside a Method**:\n   Inside any method, you can update an instance variable just by reassigning it:\n   ```python\n   def activate(self):\n       self.status = \"Online\"  # Updates THIS operative's status to Online!\n   ```\n\n3. **Independent State in Action**:\n   If you create two operatives:\n   ```python\n   op1 = Operative(\"Alpha\")\n   op2 = Operative(\"Bravo\")\n   op1.activate()  # Only op1 changes!\n   print(op1.status)  # Online\n   print(op2.status)  # Offline (Untouched!)\n   ```\n   Each object lives in its own isolated memory bubble!",
    example: "class Flashlight:\n    def __init__(self, color):\n        self.color = color\n        self.is_on = False     # Default state: starts switched off!\n\n    def switch_on(self):\n        self.is_on = True      # Changing the object's internal state!\n\nlight1 = Flashlight(\"Red\")\nlight2 = Flashlight(\"Blue\")\n\nlight1.switch_on()     # We turn on light1 only\nprint(light1.is_on)    # Outputs: True  (Turned on!)\nprint(light2.is_on)    # Outputs: False (Still off! Independent memory!)",
    task: "### YOUR MISSION 🎯\n\nGive operatives an operational status tracker that changes when activated!\n\n1. Inside `__init__`, set a default instance variable: `self.status = \"Offline\"`.\n2. Add a new method named `activate(self)` to the `Operative` class.\n3. Inside `activate`, change the status variable to: `self.status = \"Online\"`.",
    baseCode: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n        # TODO: Set self.status to \"Offline\"\n    # TODO: Add activate(self) method\n",
    hints: [
      "Add 'self.status = \"Offline\"' indented 8 spaces inside __init__.",
      "Define 'def activate(self):' indented 4 spaces, and put 'self.status = \"Online\"' inside it."
    ],
    solution: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n        self.status = \"Offline\"\n    def activate(self):\n        self.status = \"Online\"",
  ...
  },
{
    ...
    description: "Learn how objects interact, pass data, and communicate with one another.",
    headerPrefix: "INTERACTION",
    missionPrefix: "NODE_HANDSHAKE",
    intro: "# Objects Working Together: Interaction 🤝\n\nIn the real world and in software, objects don't live in solitary confinement:\n- A `Player` casts a spell on an `Enemy` 🧙‍♂️⚔️\n- A `Customer` buys an `Item` from a `Shop` 🛒\n- A `Doctor` heals a `Patient` 🩺\n\nHow do two objects communicate in Python? **You can pass an entire object as an argument into another object's method!**\n\nOnce an object arrives inside a method, you can inspect its variables and trigger its actions just like any other variable.",
    technical: "### How Object-to-Object Communication Works 📡\n\nLook at this method header:\n```python\ndef scan(self, other):\n    # 'self' is the operative DOING the scanning (the caller)\n    # 'other' is the operative BEING scanned (the target)!\n```\n\n- **`self`**: Refers to whoever called the method. In `agent1.scan(agent2)`, `self` is `agent1`.\n- **`other`**: Refers to the object passed in between the parentheses. In `agent1.scan(agent2)`, `other` is `agent2`.\n\n### Accessing the Other Object's Data:\nBecause `other` is also an `Operative`, it has its own `.designation` attribute! You can read its data with dot notation:\n`other.designation`\n\nInside an f-string, you can easily display their name:\n`f\"Targeting: {other.designation}\"`",
    example: "class Fighter:\n    def __init__(self, name, power):\n        self.name = name\n        self.power = power\n\n    # Takes another Fighter object as the 'target' parameter:\n    def attack(self, target):\n        print(f\"{self.name} strikes {target.name} for {self.power} damage!\")\n\np1 = Fighter(\"Shadow\", 25)\np2 = Fighter(\"Golem\", 10)\n\n# p1 attacks p2:\np1.attack(p2)  # Output: Shadow strikes Golem for 25 damage!",
    task: "### YOUR MISSION 🎯\n\nAllow an operative to scan another operative on the network grid!\n\n1. Inside the `Operative` class, define a method named `scan` that accepts two parameters: `self` and `other`.\n2. Inside `scan`, print this exact f-string message: `f\"Targeting: {other.designation}\"`",
    baseCode: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n    # TODO: Add scan method accepting (self, other)\n",
    hints: [
      "Define 'def scan(self, other):' indented 4 spaces inside the class.",
      "Inside scan (indented 8 spaces), write: print(f\"Targeting: {other.designation}\")"
    ],
    solution: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n    def scan(self, other):\n        print(f\"Targeting: {other.designation}\")",
    solutionRegex: [/def\s+scan\s*\(\s*self\s*,\s*other\s*\)\s*:/, /print\s*\(\s*(?:f['"]Targeting:\s*\{\s*other\.designation\s*\}['"]|['"]Targeting:\s*['"]\s*,\s*other\.designation|['"]Targeting:\s*['"]\s*\+\s*other\.designation)\s*\)/]
  },
{
    id: "oop-inheritance",
    level: "oop",
    title: "Reusing Code: Class Inheritance",
    description: "Create specialized child classes that automatically inherit abilities from a parent class.",
    headerPrefix: "INHERITANCE",
    missionPrefix: "CLASS_FAMILY",
    intro: "# Don't Repeat Yourself: Inheritance 🧬\n\nProgrammers have a famous guiding principle called **DRY**: *\"Don't Repeat Yourself\"*.\n\nImagine you spent hours building a complete `Operative` class with code designations, communication pings, and security clearances. Now, you need to create a specialized `SecurityExpert` who can do all of that, PLUS encrypt sensitive documents.\n\nWould you copy and paste 50 lines of code into a new class? Definitely not! If you found a bug later, you'd have to fix it in two different places.\n\nIn OOP, **Inheritance** lets a **Child class** automatically inherit all attributes and methods from a **Parent class** for free—while letting you add brand-new specialized powers!",
    technical: "### How Inheritance Works in Python 🌳\n\nTo make a class inherit from another, you simply write the parent class's name inside parentheses right after the child class's name:\n```python\nclass ChildClass(ParentClass):\n    # Automatically gets everything from ParentClass for free!\n    # Define child's special powers here\n```\n\n### Real-World Hierarchy:\n- **Parent (Base Class)**: `Vehicle` has wheels, engine, and a `drive()` method.\n- **Child (Subclass)**: `FireTruck(Vehicle)` automatically gets wheels and `drive()`, but adds a `spray_water()` method and a siren!\n\nIn our code:\n- `Operative` is the **Parent** (has `designation` and `__init__`).\n- `SecurityExpert` is the **Child** (inherits `designation`, but adds an `encrypt` method).",
    example: "# Parent class (General blueprint)\nclass Animal:\n    def __init__(self, name):\n        self.name = name\n    def sleep(self):\n        print(f\"{self.name} is sleeping. Zzz...\")\n\n# Child class (Specialized - inherits from Animal):\nclass Duck(Animal):\n    def quack(self):\n        print(f\"{self.name} says Quack!\")\n\ndonald = Duck(\"Donald\")\ndonald.sleep()  # Inherited from Animal! (Donald is sleeping. Zzz...)\ndonald.quack()  # Unique ability from Duck! (Donald says Quack!)",
    task: "### YOUR MISSION 🎯\n\nCreate a specialized security operative using class inheritance!\n\n1. Leave the base `Operative` class untouched.\n2. Below it, define a new child class `SecurityExpert` that inherits from `Operative`: `class SecurityExpert(Operative):`\n3. Inside `SecurityExpert`, add an `encrypt(self)` method that prints: `\"Data secured\"`",
    baseCode: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n\n# TODO: Define SecurityExpert inheriting from Operative\n",
    hints: [
      "Write 'class SecurityExpert(Operative):' with no indentation on a new line.",
  ...
  },
{
    ...
    description: "Make your objects print clean, friendly text instead of cryptic memory addresses.",
    headerPrefix: "MAGIC_METHODS",
    missionPrefix: "HUMAN_PRINT",
    intro: "# Giving Your Objects a Friendly Voice: __str__ 🗣️\n\nHave you ever tried to print a Python object directly? Try this in your head:\n```python\nagent = Operative(\"Ghost\")\nprint(agent)\n```\nWhat does Python display? It prints something confusing and ugly like:\n`<__main__.Operative object at 0x7fa4e82b13d0>`\n\nWhat is that weird text? It is a raw computer memory address! To a player, teacher, or user, that looks like broken code.\n\nPython gives us a special magic method to fix this: **`__str__`**. It tells Python: *\"Whenever someone tries to print me or convert me to a string, display this friendly description instead!\"*",
    technical: "### The #1 Golden Rule of `__str__`: RETURN, Don't Print! ⚠️\n\nLook very closely at how `__str__` is defined:\n```python\ndef __str__(self):\n    return f\"Operative {self.designation}\"  # MUST RETURN, NOT PRINT!\n```\n\n❌ **Common Beginner Mistake**: Writing `print(...)` inside `__str__`.\n✅ **The Correct Way**: You must use the **`return`** keyword to hand the finished string back to Python.\n\n### Why does it need `return`?\nWhen someone runs `print(agent)`, Python executes `str(agent)` under the hood. That calls your `__str__()` method and waits for it to **return** a string to display.\nIf you use `print()` inside `__str__`, it prints early at the wrong time and returns `None`, causing confusing bugs!",
    example: "class Book:\n    def __init__(self, title, author):\n        self.title = title\n        self.author = author\n\n    # Without __str__: print(my_book) -> <Book object at 0x102>\n    # With __str__:    print(my_book) -> 'The Hobbit' by J.R.R. Tolkien\n    def __str__(self):\n        return f\"'{self.title}' by {self.author}\"\n\nnovel = Book(\"The Hobbit\", \"J.R.R. Tolkien\")\nprint(novel)  # Friendly human-readable text!",
    task: "### YOUR MISSION 🎯\n\nGive your `Operative` class a clean, human-friendly text display when printed!\n\n1. Inside the `Operative` class, define the special method: `def __str__(self):`\n2. Inside the method, **return** this formatted string: `f\"Operative {self.designation}\"`\n*(Remember: use `return`, do NOT use `print` inside `__str__`!)*",
    baseCode: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n    # TODO: Add __str__ method that returns f\"Operative {self.designation}\"\n",
    hints: [
      "Define 'def __str__(self):' indented 4 spaces inside Operative.",
      "Inside the method (indented 8 spaces), write: return f\"Operative {self.designation}\""
    ],
    solution: "class Operative:\n    def __init__(self, designation):\n        self.designation = designation\n    def __str__(self):\n        return f\"Operative {self.designation}\"",
    solutionRegex: [/def\s+__str__\s*\(\s*self\s*\)\s*:/, /return\s+.*Operative.*self\.designation/]
  },
{
    id: "oop-class-vars",
    level: "oop",
    title: "Shared Constants: Class Variables",
    description: "Learn the difference between individual instance variables and shared class-wide variables.",
    headerPrefix: "CLASS_WIDE",
    missionPrefix: "GLOBAL_CONST",
    intro: "# One Value Shared by Everyone: Class Variables 🌐\n\nUp until now, every variable we created used `self.something` inside `__init__`. Those are called **Instance Variables**.\n- That means every individual object has its own personal copy (`agent_a` has designation \"Ghost\", `agent_b` has designation \"Viper\").\n\nBut what if you have a value that is **universal and identical across EVERY object** made from that class?\n- All humans belong to the species `\"Homo sapiens\"` 🌍\n- All game characters in a world share `gravity = 9.8` 🎮\n- All operatives on our cyber network share `security_level = \"High\"` 🛡️\n\nIn Python, a variable that belongs to the whole class as a unit is called a **Class Variable**!",
    technical: "### Instance Variables vs. Class Variables: The Showdown 🥊\n\n| Feature | Instance Variable (`self.designation`) | Class Variable (`security_level`) |\n|---|---|---|\n| **Where is it defined?** | Inside `__init__` using `self.` | Directly inside the `class` block, at the very top |\n| **Who owns it?** | Each individual object | The Class itself (shared by all) |\n| **How do you access it?** | `my_agent.designation` | `Operative.security_level` or `my_agent.security_level` |\n| **Real-life analogy** | Your personal mobile number | The country calling code (+44 or +1) |\n\n### How to Create and Access a Class Variable:\n```python\nclass Operative:\n    # 1. Defined at the top of the class, outside any method:\n    security_level = \"High\"\n\n# 2. You can access it directly from the class name without even making an object!\nprint(Operative.security_level)  # Outputs: High\n```",
    example: "class Spaceship:\n    # Class variable: ALL ships belong to this shared fleet\n    fleet = \"Earth Defense Coalition\"\n\n    def __init__(self, ship_id):\n        # Instance variable: EACH ship has a unique serial code\n        self.id = ship_id\n\nship1 = Spaceship(\"Cruiser-01\")\nship2 = Spaceship(\"Cruiser-02\")\n\n# Accessing via the class itself:\nprint(Spaceship.fleet)  # Earth Defense Coalition\n\n# Both ships share the exact same fleet:\nprint(ship1.fleet)      # Earth Defense Coalition\nprint(ship2.fleet)      # Earth Defense Coalition",
    task: "### YOUR MISSION 🎯\n\nEstablish a network-wide security clearance shared by all operatives!\n\n1. Inside `Operative`, directly at the top (indented 4 spaces, before any methods), create a class variable: `security_level = \"High\"`.\n2. Outside the class (with no indentation at the bottom), print the class variable using the class name: `print(Operative.security_level)`",
    ...
    hints: [
      ...
      "On a new line outside the class (no indentation), write: print(Operative.security_level)"
    ],
